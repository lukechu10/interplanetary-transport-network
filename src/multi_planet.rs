...
pub fn start() {
    ...
    let mass = array![1., 1., 1.];
    // let mass_positions_at_t = Array3::zeros((time_steps, 1, 2));
    let mass_positions_at_t = array![[-1., 0.], [0., 0.], [1., 0.]]
        .broadcast((time_steps, 3, 2))
        .unwrap()
        .to_owned();

    let num_ships = 400;

    let ship_positions = array![2., 0.].broadcast((num_ships, 2)).unwrap().to_owned();
    ...
    write_npy("data/multi_planet_bodies.npy", &mass_positions_at_t).unwrap();
    write_npy("data/multi_planet_ships.npy", &ship_positions).unwrap();
    write_npy(
